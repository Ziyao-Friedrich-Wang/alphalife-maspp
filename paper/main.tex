...
Code, run scripts, result snapshots, and a synthetic-data smoke test are available at \url{https://github.com/Ziyao-Friedrich-Wang/alphalife-maspp}. The repository does not redistribute the empirical factor-return panel, but it documents the expected data layout, reproduces the committed tables from result snapshots, and provides a deterministic smoke test that verifies the core lifecycle allocation pipeline without external data.
